...
function showTab(id,el){document.querySelectorAll('.tabpane').forEach(x=>x.classList.add('hidden'));const pane=document.getElementById('tab-'+id);if(pane)pane.classList.remove('hidden');document.querySelectorAll('.side-link').forEach(x=>x.classList.remove('active'));const idx={multi:0,upload:1,processing:2,review:3,knowledge:4,experience:5,conversation:6,monitor:7,compare:8,diagnostics:9}[id];document.querySelectorAll('.side-link')[idx]?.classList.add('active');if(el)el.classList.add('active');document.getElementById('pageTitle').textContent=titleMap[id]?.[0]||'AI Center';document.getElementById('pageSub').textContent=titleMap[id]?.[1]||'';if(id==='knowledge'){safeLoad(loadKnowledge,'knowledgeItems','Lỗi tải Knowledge');safeLoad(loadAiBrainBuildStatus,'brainBuildBox','Lỗi trạng thái AI Brain');}if(id==='experience')safeLoad(loadExperiences,'experienceItems','Lỗi tải kinh nghiệm');if(id==='review')safeLoad(loadReviewItems,'reviewItems','Lỗi tải chờ duyệt');if(id==='processing')safeLoad(loadLearningItems,'learningItems','Lỗi tải tài liệu học');if(id==='monitor')safeLoad(loadReports,'reports','Lỗi tải báo cáo');if(id==='diagnostics')safeLoad(()=>runDiagnostics(false),'diagnosticsResult','Lỗi diagnostics');}
...
async function loadAiBrainBuildStatus(){
  const box=document.getElementById('brainBuildBox');
  const st=document.getElementById('brainBuildStatus');
  try{
    if(st)st.textContent=' Đang kiểm tra...';
    const data=await api('/learning/brain/status');
    const c=data.counts||{};
    const last=data.lastBuild||{};
    if(st)st.textContent=' Đã kiểm tra';
    if(box)box.innerHTML=`<div class="small-row"><span>Tổng segments</span><b>${c.total||0}</b></div><div class="small-row"><span>Đã vào AI Brain</span><b>${c.built||0}</b></div><div class="small-row"><span>Đã hấp thụ</span><b>${c.absorbed||0}</b></div><div class="small-row"><span>Hấp thụ một phần</span><b>${c.partial||0}</b></div><div class="small-row"><span>Cần OCR/Parser</span><b>${c.needsExtraction||0}</b></div><div class="small-row"><span>Inactive</span><b>${c.inactive||0}</b></div>${last.builtAt?`<p>Build gần nhất: <b>${escapeHtml(last.builtAt)}</b> • Cập nhật ${last.updated||0} • Khôi phục ${last.reactivated||0}</p>`:''}`;
  }catch(e){ if(st)st.textContent=' Lỗi kiểm tra AI Brain'; if(box)box.innerHTML=`<p style="color:#b91c1c">${escapeHtml(e.message||String(e))}</p>`; }
}
async function buildAiBrainFromKnowledge(){
  const ok=confirm('Xây dựng AI Brain từ toàn bộ Knowledge hiện có trong Supabase? Hệ thống sẽ khôi phục các segment bị inactive và đánh dấu để Bot/AI Compare truy xuất.');
  if(!ok)return;
  const box=document.getElementById('brainBuildBox');
  const st=document.getElementById('brainBuildStatus');
  try{
    if(st)st.textContent=' Đang xây dựng AI Brain...';
    if(box)box.innerHTML='<p>Đang quét Knowledge, chuẩn hóa Object và cập nhật Supabase. Vui lòng chờ...</p>';
    const data=await api('/learning/brain/build',{method:'POST',body:JSON.stringify({includeInactive:true,limit:20000})});
    if(st)st.textContent=' Hoàn tất xây dựng AI Brain';
    if(box)box.innerHTML=`<p><b>Đã xong.</b></p><div class="small-row"><span>Đã quét</span><b>${data.scanned||0}</b></div><div class="small-row"><span>Đã cập nhật vào AI Brain</span><b>${data.updated||0}</b></div><div class="small-row"><span>Đã khôi phục inactive</span><b>${data.reactivated||0}</b></div><div class="small-row"><span>Lỗi/bỏ qua</span><b>${(data.errors||[]).length+(data.skipped||0)}</b></div><details><summary>Chi tiết loại Object</summary><pre>${escapeHtml(JSON.stringify(data.byType||{},null,2))}</pre></details>`;
    await loadKnowledge(); await loadSummary();
  }catch(e){ if(st)st.textContent=' Lỗi xây dựng AI Brain'; if(box)box.innerHTML=`<p style="color:#b91c1c">${escapeHtml(e.message||String(e))}</p>`; }
}
