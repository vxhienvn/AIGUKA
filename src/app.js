...
let BOT_REPLY_ENABLED = typeof localBotReplyDefault === 'boolean'
    ...
    : String(process.env.BOT_REPLY_ENABLED || 'false').toLowerCase() === 'true';
...
// ===== V7.4.3 SAFETY: hard outbound gate =====
// Default false for sync/testing builds. This blocks ALL outgoing Messenger sends, including force/template/image.
...
async function logMessageToSupabase({ event = null, senderId = "", pageId = "", role = "customer", text = "", messageType = "text", raw = null, productGroup = "", intent = "", attachmentUrl = "", source = "", externalMessageId = "", bypassActiveServerGuard = false }) {
    if (!supabaseIsReady()) return { skipped: true };
    if (!bypassActiveServerGuard && !(await requireActiveServerForMutation("log_message_to_supabase"))) {
...
async function graphFetchUrlJson(url) {
    const response = await fetch(url);
    const raw = await response.text();
    let data = null;
    try { data = raw ? JSON.parse(raw) : null; } catch (_) { data = { raw }; }
    if (!response.ok) {
        const compact = compactExternalErrorBody(raw);
        const e = new Error(`Graph paging failed ${response.status}: ${compact}`);
        e.status = response.status;
        e.compact_body = compact;
        throw e;
    }
...
async function graphFetchAllPages(pathname, params = {}, { maxItems = 500, maxPages = 20 } = {}) {
    const first = await graphFetchJson(pathname, params);
    const items = [];
    let page = first;
    let pages = 0;
    while (page && pages < maxPages && items.length < maxItems) {
        ...
        if (!nextUrl || items.length >= maxItems) break;
        page = await graphFetchUrlJson(nextUrl);
        pages++;
    }
    return { data: items, first, pages: pages + 1 };
}

async function fetchMessengerThreadMessages(threadId, { limitMessages = 500 } = {}) {
    const maxMessages = Math.min(Math.max(Number(limitMessages) || 500, 1), 1000);
    const fields = 'id,message,from,to,created_time,attachments';
    const result = await graphFetchAllPages(`${threadId}/messages`, { fields, limit: 100 }, { maxItems: maxMessages, maxPages: Math.ceil(maxMessages / 100) + 2 });
    return result.data || [];
}
...
async function messengerFindThreadsForSender(senderId, limitMessages = 500) {
    const fields = 'id,updated_time,participants';
    const attempts = [];
    // user_id query thường là cách nhanh nhất để tìm thread theo PSID.
    try {
        const direct = await graphFetchJson('me/conversations', { platform: 'messenger', user_id: senderId, fields, limit: 10 });
    ...
    // Fallback: quét nhiều thread gần nhất rồi lọc participant.
    try {
        const recent = await graphFetchAllPages('me/conversations', { platform: 'messenger', fields, limit: 100 }, { maxItems: 500, maxPages: 8 });
        const threads = (recent?.data || []).filter(t => JSON.stringify(t).includes(String(senderId)));
        attempts.push({ mode: 'recent_filter_paginated', count: threads.length, scanned: (recent?.data || []).length });
        return { threads, attempts };
    } catch (err) {
        attempts.push({ mode: 'recent_filter_paginated', error: err.message });
    }
    return { threads: [], attempts };
}
...
async function syncMessengerThreadBySender(senderId, { limitMessages = 20, force = false } = {}) {
    ...
    for (const thread of found.threads) {
        const messages = await fetchMessengerThreadMessages(thread.id, { limitMessages });
...
async function syncRecentMessengerConversations({ limit = 50, limitMessages = 500 } = {}) {
    if (!MESSENGER_SYNC_ENABLED) return { ok: false, skipped: true, reason: 'MESSENGER_SYNC_DISABLED' };
    if (!PAGE_ACCESS_TOKEN) return { ok: false, skipped: true, reason: 'PAGE_ACCESS_TOKEN_MISSING' };
    const fields = 'id,updated_time,participants';
    const maxThreads = Math.min(Math.max(Number(limit) || 50, 1), 500);
    const maxMessages = Math.min(Math.max(Number(limitMessages) || 500, 1), 1000);
    let data;
    try {
        data = await graphFetchAllPages('me/conversations', { platform: 'messenger', fields, limit: 100 }, { maxItems: maxThreads, maxPages: Math.ceil(maxThreads / 100) + 2 });
    } catch (error) {
        return {
            ok: false,
            source: 'graph',
            ...
            hint: 'Graph lỗi hoặc token/permission chưa ổn. Cần pages_messaging/pages_show_list/pages_read_engagement và PAGE_ACCESS_TOKEN của đúng Page.'
        };
    }
    const result = { ok: true, mode: 'recent_paginated', conversations: Array.isArray(data?.data) ? data.data.length : 0, messages_seen: 0, messages_logged: 0, admin_seen: 0, customer_seen: 0, bot_seen: 0, pancake_auto_seen: 0, sources: {}, errors: [] };
    for (const thread of data?.data || []) {
        let messages = [];
        try {
            messages = await fetchMessengerThreadMessages(thread.id, { limitMessages: maxMessages });
        } catch (err) {
            result.errors.push(`thread ${thread.id} messages: ${err.message}`);
            continue;
        }
        for (const msg of [...messages].reverse()) {
            try {
...
app.get('/api/debug/health', async (req, res) => {
    ...
    res.json({
        ...
        endpoints: [
            ...
            'POST /api/sync/messenger?limit=50&messages=500',
            'POST /api/sync/messenger/sender/:senderId?messages=500',
            'POST /api/sync/messenger/conversation/:conversationId?messages=1000'
        ]
    });
});
...
function isVietnamesePhoneLike(raw = '') {
    const d = String(raw || '').replace(/\D/g, '');
    return /^0\d{9}$/.test(d) || /^84\d{9}$/.test(d) || /^\+?84\d{9}$/.test(String(raw || '').replace(/\s+/g, ''));
}

function containsDangerousHallucinatedPrice(text = '') {
    const raw = String(text || '');
    const norm = normalizeIntentText(raw);
    if (/(\b900\s*triệu\b|\b900\s*trieu\b|\b1\s*tỷ\b|\b1\s*ty\b|\bmột\s*tỷ\b|\bmot\s*ty\b)/i.test(raw)) return true;
    if (/900\s*trieu|1\s*ty|mot\s*ty/.test(norm)) return true;
    if (/khoang\s*9[0-9]{2}\s*[-–]\s*9[0-9]{2}\s*trieu/i.test(norm)) return true;
    if (/hotline[^\n]{0,60}(triệu|trieu|tỷ|ty)/i.test(raw)) return true;
    return false;
}

function buildSafeReplyForDangerousPrice(senderId, state = {}, source = '') {
    const customerText = getLastCustomerTextForGateway(senderId, state);
    const msg = String(customerText || '').toLowerCase();
    if (/(địa\s*chỉ|dia\s*chi|ở\s*đâu|o\s*dau|showroom|cửa\s*hàng|cua\s*hang|qua\s*xem)/i.test(msg)) {
        const address = process.env.SHOP_ADDRESS || '254 Phố Keo, Gia Lâm, Hà Nội';
        const hotline = process.env.SHOP_HOTLINE || '0973693677';
        return `Dạ showroom bên em ở ${address} ạ. Hotline bên em là ${hotline}. Mình qua xem mẫu thì nhắn trước giúp em để bên em hướng dẫn đúng khu vực hàng ạ.`;
    }
    if (isPriceInquiryText(customerText)) {
        ...
        return buildPriceGuidanceReply(productType, customerText);
    }
    return 'Dạ vâng ạ. Thông tin giá chi tiết bên em cần kiểm tra đúng mẫu và chương trình hiện tại, em không báo bừa trên Messenger. Mình cho em biết nhóm sản phẩm/mẫu đang quan tâm để em hỗ trợ đúng hơn ạ.';
}

function softenExactPricesForCustomer(text = '') {
    let out = String(text || '');
    out = out.replace(/(?:giá\s*)?(\d{1,3}(?:[\.,]\d{3})+|\d+(?:[\.,]\d+)?)\s*(đ|vnd|vnđ|triệu|tr|trieu)\b/gi, (m, num, unit) => {
        const raw = String(m || '');
        if (isVietnamesePhoneLike(raw) || /hotline|sđt|sdt|zalo|phone/i.test(raw)) return raw;
        const nums = raw.replace(/[^0-9]/g, '');
        if (!nums) return raw;
        let n = Number(nums);
        if (/triệu|tr|trieu/i.test(unit) && n < 1000) n = n * 1000000;
        ...
        if (low >= 100) return 'giá cần kiểm tra theo đúng mẫu';
...
function validateAndRewriteOutboundReply(senderId, text = "", state = {}, source = "") {
    ...
    if (containsDangerousHallucinatedPrice(out)) {
        const rewritten = buildSafeReplyForDangerousPrice(senderId, state, source);
        console.warn("[RESPONSE_VALIDATOR_DANGEROUS_PRICE_BLOCK]", JSON.stringify({ senderId: String(senderId), source, from: out.slice(0, 180), to: rewritten.slice(0, 180) }));
        out = rewritten;
    }
    ...
    if (containsDangerousHallucinatedPrice(out)) {
        const rewritten = buildSafeReplyForDangerousPrice(senderId, state, source + ':post_soften');
        console.warn("[RESPONSE_VALIDATOR_DANGEROUS_PRICE_BLOCK_AFTER_SOFTEN]", JSON.stringify({ senderId: String(senderId), source, from: out.slice(0, 180), to: rewritten.slice(0, 180) }));
        out = rewritten;
    }
...
async function loadWorkingSettingsFromSupabase() {
    ...
    try {
        ...
        warnSchemaCompatOnce('app_settings:sale_center_config', `[SALE_CENTER] app_settings chưa sẵn sàng hoặc thiếu bảng; fallback bot_working_settings: ${compactSupabaseErrorMessage(error)}`);
    }
...
function createAdSession(state, adCtx, now = Date.now()) {
    ...
    if (adCtx.product) {
        // AIGUKA 7.2.6: Ads Mapping chỉ tạo phạm vi/ngữ cảnh quảng cáo, không tự khóa current product.
        // Product cụ thể chỉ khóa khi khách nói rõ hoặc khi Business Rule cần dùng scope để gửi slide nhóm.
        state.adProductScope = adCtx.product;
        state.lastAdScope = adCtx.product;
        state.lastAdScopeSource = adCtx.raw_mapping ? "ad_mapping" : "ad_text";
    }
    return session;
}
...
function resolveWorkflowProduct(state, customerMessage = "", historyText = "", event = {}) {
    ...
    // AIGUKA 7.2.6: Nếu khách không nói rõ sản phẩm, Ads Mapping chỉ được lưu làm scope.
    // Không trả về fromSession/fromAd như product cụ thể, vì sẽ gây lỗi QC tổng hợp -> tự gán bồn cầu/lavabo/quạt.
    ...
    const scopeOnly = fromSession || fromAd || state.adProductScope || state.lastAdScope || null;
    if (scopeOnly) {
        state.adProductScope = scopeOnly;
        state.lastAdScope = scopeOnly;
    }
...
function detectExplicitProductForDecision(message = '') {
    const explicitItem = detectProductItemFromTextAnyGroup(message);
    const explicitFromItem = explicitItem ? inferProductTypeFromProductItem(explicitItem) : null;
    const explicitTopic = detectExplicitTopic(message);
    const numeric = groupFromNumericChoice(message);
    return {
        product: explicitFromItem || explicitTopic || numeric || null,
        productItemKey: explicitItem?.product_item_key || '',
        productItem: explicitItem || null,
        source: explicitFromItem || explicitTopic || numeric ? 'customer_explicit' : 'none'
    };
}

function isInfoRequestIntent(message = '', intent = '') {
    const txt = normalizeIntentText(message);
    return intent === 'ask_features' || /\b(thong tin|tu van|chi tiet|gioi thieu|mau ma|catalog|catalogue|bao gia|gia sao|gia the nao)\b/i.test(txt);
}
...
function decideBotActionV726({ event = {}, customerMessage = '', state = {}, history = [], now = Date.now() } = {}) {
    ...
    if (isNoSlideServiceIntent(intent)) {
        return {
            ...
            reason: 'service_intent_has_priority_over_ad_mapping'
        };
    }
    ...
    return { handled: false, action: 'allow_v5_or_legacy', allowAI: true, intent, product: explicit.product || null, adScope: adScope.scope };
}
...
async function executeBotActionV726(senderId, event, customerMessage, state, decision, history = []) {
    ...
    if (decision.action === 'contact_handover') {
        ...
        saveConversations(conversations);
        saveCustomerStates(customerStates);
        logMessageToSupabase({ event, senderId, pageId, role: 'bot', text: reply, messageType: 'text', productGroup: toDbProductGroup(productType), intent: 'contact_collected', source: 'v726_decision_engine', raw: decision }).catch(() => {});
        return true;
    }
    ...
    if (decision.action === 'send_slide_then_ask_phone' || decision.action === 'send_scope_slide_for_price_then_ask_phone' || decision.action === 'send_scope_slide_for_info_then_ask_phone' || decision.action === 'product_showcase_then_ask_phone') {
        ...
        if (decision.productSource === 'customer_explicit') {
            lockProductForConversation(state, productType, 'v726_customer_explicit_action');
        } else {
            state.adProductScope = productType;
            state.lastAdScope = productType;
        }
        ...
        saveConversations(conversations);
        saveCustomerStates(customerStates);
        logMessageToSupabase({ event, senderId, pageId, role: 'bot', text: `${intro}\n${close}`, messageType: 'text', productGroup: toDbProductGroup(productType), intent: decision.intent, source: 'v726_decision_engine', raw: { ...decision, mediaResult } }).catch(() => {});
        return true;
    }
...
async function handleMessage(event) {
    ...
    // AIGUKA 7.2.6 Decision Engine chạy trước V5/Legacy để rule cứng không bị prompt/AI phá vỡ.
    const decisionHistoryV726 = conversations[senderId] || [];
    const decisionV726 = decideBotActionV726({ event, customerMessage, state, history: decisionHistoryV726, now });
...
app.post('/api/sync/messenger', async (req, res) => {
    if (!requireAigukaDebugAccess(req, res)) return;
    try {
        const limit = Math.min(Math.max(Number(req.query.limit || req.body?.limit) || 50, 1), 500);
        const messages = Math.min(Math.max(Number(req.query.messages || req.body?.messages) || 500, 1), 1000);
...
app.get('/api/sync/messenger', async (req, res) => {
    ...
    try {
        ...
        const messages = Math.min(Math.max(Number(req.query.messages) || 500, 1), 1000);
        const result = await syncRecentMessengerConversations({ limit, limitMessages: messages });
        res.json({ ok: true, version: AIGUKA_VERSION, result });
    } catch (error) {
...
app.post('/api/sync/messenger/conversation/:conversationId', async (req, res) => {
    if (!requireAigukaDebugAccess(req, res)) return;
    try {
        const conversationId = String(req.params.conversationId || '').trim();
        const messages = Math.min(Math.max(Number(req.query.messages || req.body?.messages) || 1000, 1), 1000);
        const fields = 'id,updated_time,participants';
        const thread = await graphFetchJson(conversationId, { fields });
        const result = { ok: true, conversation_id: conversationId, messages_seen: 0, messages_logged: 0, errors: [], sources: {}, admin_seen: 0, customer_seen: 0, bot_seen: 0 };
        const rows = await fetchMessengerThreadMessages(conversationId, { limitMessages: messages });
        ...
        console.error('[MESSENGER_SYNC_API] conversation error:', error.message);
        res.status(500).json({ ok: false, error: error.message });
    }
});

app.get('/api/sync/messenger/conversation/:conversationId', async (req, res) => {
    if (!requireAigukaDebugAccess(req, res)) return;
    try {
        const conversationId = String(req.params.conversationId || '').trim();
        const messages = Math.min(Math.max(Number(req.query.messages) || 1000, 1), 1000);
        const fields = 'id,updated_time,participants';
        const thread = await graphFetchJson(conversationId, { fields });
        const result = { ok: true, conversation_id: conversationId, messages_seen: 0, messages_logged: 0, errors: [], sources: {}, admin_seen: 0, customer_seen: 0, bot_seen: 0 };
        const rows = await fetchMessengerThreadMessages(conversationId, { limitMessages: messages });
        ...
        console.error('[MESSENGER_SYNC_API] conversation GET error:', error.message);
        res.status(500).json({ ok: false, error: error.message });
    }
});
