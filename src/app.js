...
// Default OFF for safety: the system still collects data, analyzes messages, syncs ads and updates dashboards,
// but it must not send any customer-facing text/template/image until Admin explicitly turns Bot Reply ON.
...
async function sendMessage(senderId, text, options = {}) {
    ...
    if (!isBotReplyEnabled()) {
        console.log("[BOT_REPLY_SWITCH] blocked text reply (no exceptions)", senderId, String(text || '').slice(0, 120));
...
async function sendTemplate(senderId, elements, logName, options = {}) {
    ...
    // priorityBypass chỉ bỏ qua các khóa phụ. Bot Reply OFF vẫn là khóa cuối tuyệt đối.
    ...
    if (!isBotReplyEnabled()) {
        console.log("[BOT_REPLY_SWITCH] blocked template reply (no exceptions)", senderId, logName || "template");
...
app.get('/api/bot-reply-switch', async (req, res) => {
    try {
        ...
        res.json({ success: true, reply_enabled: isBotReplyEnabled(), source: loaded.ok ? loaded.source : BOT_REPLY_SOURCE, loaded, env_default: String(process.env.BOT_REPLY_ENABLED || 'false') });
    } catch (error) {
...
function dashboardRenderMetaMonthHtml({ limit, fullTotal, report, pancakeReport = [], pancakeMeta, metaDaily, monthKey, dataSource = "meta", dateStart = "", dateEnd = "" }) {
    ...
    const rowHtml = tableRows.map((x, index) => `
    ...
    return `<!doctype html><html lang="vi"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>Báo cáo tháng theo giờ Meta</title><script src="https://cdn.jsdelivr.net/npm/xlsx/dist/xlsx.full.min.js"></script><style>
...
function exportTableExcel(tableId, fileName, sheetName){ const table=document.getElementById(tableId); if(!table){ alert('Không tìm thấy bảng để xuất Excel'); return; } if(!window.XLSX){ alert('Chưa tải được thư viện xuất Excel. Vui lòng thử lại sau vài giây.'); return; } const wb=XLSX.utils.table_to_book(table,{sheet:sheetName||'Bang'}); XLSX.writeFile(wb,(fileName||'bang_bao_cao')+'.xlsx'); } function exportAdsTableExcel(){ exportTableExcel('adsPerformanceTable','hieu_qua_quang_cao','Hieu qua quang cao'); }
...
app.get('/dashboard-meta-month', async (req, res) => {
    try {
        const limit = req.query.limit || 500;
        const fromQuery = String(req.query.from || req.query.start || "").slice(0, 10);
        const toQuery = String(req.query.to || req.query.end || "").slice(0, 10);
        const monthKey = String(req.query.month || (fromQuery ? fromQuery.slice(0, 7) : dashboardTodayKeyMeta(0).slice(0, 7)));
        const defaultSince = `${monthKey}-01`;
        const days = dashboardDaysInMonthFromKey(defaultSince);
        const todayMeta = dashboardTodayKeyMeta(0);
        const defaultUntil = monthKey === todayMeta.slice(0, 7) ? todayMeta : `${monthKey}-${String(days).padStart(2, "0")}`;
        const since = fromQuery || defaultSince;
        const until = toQuery || defaultUntil;
