...
async function splitPdfFile(file, maxPartMb=25){
  ...
  const maxBytes=maxPartMb*1024*1024;
  const parts=[];
  let page=0;
  let chunkPages=Math.max(1, Math.ceil(totalPages * maxBytes / Math.max(file.size,1)));
  chunkPages=Math.min(chunkPages,30);
  while(page<totalPages){
    let take=Math.min(chunkPages,totalPages-page);
    let outBytes=null;
    while(take>=1){
      const doc=await PDFDocument.create();
      const indexes=Array.from({length:take},(_,i)=>page+i);
      const copied=await doc.copyPages(src,indexes);
      copied.forEach(pg=>doc.addPage(pg));
      doc.setTitle(`${file.name} part ${parts.length+1}`);
      outBytes=await doc.save({useObjectStreams:true});
      if(outBytes.byteLength<=maxBytes || take===1)break;
      take=Math.max(1,Math.floor(take/2));
    }
    parts.push({bytes:outBytes,pageStart:page+1,pageEnd:page+take});
    page+=take;
  }
  const total=parts.length;
  return parts.map((p,i)=>new File([p.bytes], makePartName(file.name,i+1,total), {
...
async function uploadLearningFiles(){
  ...
  const blocked=files.filter(f=>uploadRisk(f).blocked);
  if(status)status.innerHTML=renderUploadPlan(files);
  if(blocked.length){
    const names=blocked.map(f=>`${f.name} (${formatBytes(f.size)})`).join('\n');
    const msg='Có file quá lớn nên hệ thống chưa upload để tránh treo/520. Hãy tách hoặc nén file trước khi cho AI học:\n'+names;
    if(status)status.insertAdjacentHTML('beforeend',`<div class="report red card" style="padding:10px;margin-top:8px"><b>Chưa upload.</b><br>${escapeHtml(msg).replace(/\n/g,'<br>')}<br><button class="btn secondary" style="margin-top:8px" onclick="splitLargePdfFiles()">Tách PDF lớn ngay</button></div>`);
    alert(msg);
    return;
  }
  ...
  for(let i=0;i<files.length;i++){
    ...
    try{
      ...
      ok++;
      if(status)status.innerHTML=`${renderUploadPlan(files)}<div class="report green card" style="padding:10px;margin-top:8px">✅ Đã upload ${ok}/${files.length}: <b>${escapeHtml(file.name)}</b>. Đang chuyển sang xử lý/nhận diện.</div>`;
    }catch(e){
      fail++;
      const hint=String(e.message||'').includes('không trả JSON')||String(e.message||'').includes('520')?'Backend/proxy trả HTML, thường do file quá lớn, route chưa deploy hoặc server restart. Thử Ctrl+F5, kiểm tra deploy, hoặc tách file nhỏ hơn.':'';
  ...
  document.getElementById('learningFiles').value='';
  if(status)status.innerHTML=`${renderUploadPlan(files)}<div class="report ${fail?'orange':'green'} card" style="padding:10px;margin-top:8px">Hoàn tất: ✅ ${ok} thành công, ❌ ${fail} lỗi.</div>`;
